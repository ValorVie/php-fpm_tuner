...
- [ ] 1.1 建立目錄結構 (`bin/`, `lib/`, `config/`)
- [ ] 1.2 建立 `lib/bootstrap.php`（polyfills、require 所有 lib）
- [ ] 1.3 建立 `config/default.php`（從現有 `$config` 抽取）

## 2. 核心模組抽取 (Phase 2)

- [ ] 2.1 建立 `lib/SystemInfo.php`
  - 從 php-fpm-tuner.php 抽取 getCpuCores()、getFreeMemory()、getWorkerMemory()
  - 新增 `SystemInfo::collect()` 靜態方法

- [ ] 2.2 建立 `lib/Config.php`
  - `Config::load($path)` - 載入配置檔
  - `Config::merge($default, $override)` - 合併配置
  - `Config::fromCliArgs($argv)` - 解析 CLI 參數

- [ ] 2.3 建立 `lib/Calculator.php`
...
- [ ] 5.1 修改 `php-fpm-tuner.php` 為包裝入口
  - 保留原檔名
  - 內部呼叫 `bin/tuner`
  - 確保原有輸出格式不變
...
- [ ] 6.2 更新 docs/EVALUATION.md
  - 更新腳本路徑
  - 新增使用範例

- [ ] 6.3 新增 docs/ARCHITECTURE.md
  - 模組說明
  - API 參考
...
- [ ] 所有 bin 腳本可獨立執行
- [ ] `php php-fpm-tuner.php` 輸出與重構前一致
- [ ] PHP 7.1 環境可正常執行
- [ ] 無外部相依（純 PHP 標準函式庫）
